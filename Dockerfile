...
RUN curl http://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz -o /tmp/nginx-${NGINX_VERSION}.tar.gz && cd /tmp && tar xvzf nginx-${NGINX_VERSION}.tar.gz
...
FROM nginx:${DOCKER_NGINX_VERSION}-${DOCKER_DEBIAN_FLAVOR}
RUN apt update && \
    apt-get upgrade -y \ 
    libssl-dev \
    zlib1g-dev \
